(() => {
  ...
  function setupIntro() {
    ...
    if (reducedMotion) {
      ...
      if (preloader) {
        preloader.classList.add('is-leaving');
        window.setTimeout(() => { preloader.hidden = true; }, 80);
      }
      return;
    }

    // Mantém a assinatura visual da abertura sem segurar o usuário por vários segundos.
    // O preloader fica visível por ~0,7 s e sai enquanto o hero começa a entrar em sequência.
    window.setTimeout(() => {
      preloader?.classList.add('is-leaving');
    }, 680);
    ...
    // Failsafe: o overlay nunca pode permanecer preso, mesmo em navegador lento.
    window.setTimeout(() => {
      document.body.classList.add('site-ready');
      if (preloader) {
        preloader.classList.add('is-leaving');
